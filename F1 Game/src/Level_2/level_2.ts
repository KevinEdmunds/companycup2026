...
function pickBestTyre(level: LevelFile, weather: WeatherCondition): { id: number; friction: number } {
  ...
  for (const set of level.available_sets) {
    ...
    if (!best || friction > best.friction) best = { id: firstId, friction };
  }
 ...
 */
function computeSpeedCaps(segments: Segment[], friction: number, maxSpeed: number, crawl: number, brake: number): number[] {
  const n = segments.length;
  const entryCap = new Array<number>(n).fill(maxSpeed);
  const exitCap = new Array<number>(n).fill(maxSpeed);
 ...
 */
function segmentFuel(kBase: number, s: SegInfo): number {
  if (s.type === "corner") {
...
function planRefuelStops(
  ...
  const stintLaps: number[] = Array.from({ length: stintCount }, (_, i) => base + (i < remainder ? 1 : 0));

  // Lap number on which each stint ends; we pit at the end of all but the last.
  const stintEndLap: number[] = [];
  let acc = 0;
  ...
  const refuelByLap = new Map<number, number>();
  ...
  for (let s = 0; s < stintCount; s += 1) {
    ...
    if (pitLap !== undefined) refuelByLap.set(pitLap, ceil2(refuel));
    fuel += refuel;
  }
...
function buildStrategy(level: LevelFile): { output: unknown; summary: string } {
  ...
  const segmentActions: Array<StraightAction | CornerAction> = steady.segs.map((s) =>
    s.type === "straight"
      ? {
          id: s.id,
          type: "straight",
  ...
  const summary =
    `Tyre: ${tyre.id} | per-lap fuel: ${perLapFuel.toFixed(3)} L | total fuel used: ${totalFuel.toFixed(1)} L\n` +
